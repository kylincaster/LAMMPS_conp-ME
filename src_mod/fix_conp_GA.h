...
class FixConpGA : public Fix {
 public:
  ...
  void pcg_Matrix_Residual(double, double *const, const double *const, const double *const,
  ...
  int Ele_Cell_Model;    // Model of CELL
  ...
  std::vector<double> res_localGA_vec, ap_localGA_vec, p_localGA_vec, z_localGA_vec,
      z_localGA_prv_vec;
  ...
  int Xi_self_index;
  int symmetry_flag;          // Wether or not to make a symmetric matrix
  ...
  int pcg_mat_sel;            // stype of precondition matrix: normal, full, pseudo
  ...
  int AAA_save_format, minimizer;
  int tol_style, B0_refresh_time, qsqsum_refresh_time;
 ...
 private:
  ...
  void setup_GG_Flag();
  void calc_B0(double *);
  void make_symmetric_matrix(int flag, int n, double **mat);
  void energy_clear();
