...
// clang-format off
KSpaceStyle(pppm_conp/GA, PPPM_conp_GA)
// clang-format on

#else

#ifndef LMP_PPPM_CONP_GA_H
#define LMP_PPPM_CONP_GA_H
...
namespace LAMMPS_NS
{
class PPPM_conp_GA : public KSpace, public KSpaceConpGA {
public:
  PPPM_conp_GA(class LAMMPS*);
  virtual ~PPPM_conp_GA();
  ...
  /* --------------- For PPPM_conp part --------------- */
  virtual const char* type_name() { return "PPPM_conp/GA"; };
  virtual double get_qsum() { return qsum; };
  virtual double get_qsqsum() { return qsqsum; };
  virtual void set_qsqsum(double qsum0, double qsqsum0)
  {
    qsum   = qsum0;
    qsqsum = qsqsum0;
    q2     = qsqsum * qqrd2e;
  ...
  virtual void compute_GAtoGA();
  virtual void clear_saved();
  // virtual bool is_compute_AAA() { return false; }
  /* --------- End of For PPPM_conp/GA part ---------- */

protected:
  virtual void compute_dipole(int groupbit, bool Select_Flag);
  virtual void calc_GA_POT(double, bool);
  double dipole_group, dipole_r2_group;
  int me, nprocs;
  int nfactors;
  int* factors;
  double cutoff;
  ...
  int peratom_allocate_flag, init_times;
  ...
  double qSOL_local;
  FFT_SCALAR** GA2rho;
  FFT_SCALAR*** SOL_density_brick;
  // FFT_SCALAR ***GA_density_brick;
  bool sv_GA_Flag, sv_SOL_Flag, sv_SOL_saved, fixed_flag;
  ...
  virtual void particle_map(int, bool);
  virtual void clear_rho();
  virtual void add_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src);
  virtual void sub_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src);
  virtual void copy_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src);
  virtual double make_rho(int, bool);
  virtual double make_rho_GA();
  virtual double map_make_rho_SOL(int);
  virtual void setup_GA2rho();
