...
/* -*- c++ -*- -------------------------------------------------------------
   ...
   pppm_conp/GA: PPPM class to perform pppm calculation for Conp/GA
------------------------------------------------------------------------- */
...
PPPM_conp_GA::PPPM_conp_GA(LAMMPS* lmp)
    : KSpace(lmp), factors(nullptr), density_brick(nullptr), vdx_brick(nullptr), vdy_brick(nullptr), vdz_brick(nullptr), u_brick(nullptr), v0_brick(nullptr),
      v1_brick(nullptr), v2_brick(nullptr), v3_brick(nullptr), v4_brick(nullptr), v5_brick(nullptr), greensfn(nullptr), vg(nullptr), fkx(nullptr), fky(nullptr),
      fkz(nullptr), density_fft(nullptr), work1(nullptr), work2(nullptr), gf_b(nullptr), rho1d(nullptr), rho_coeff(nullptr), drho1d(nullptr),
      drho_coeff(nullptr), sf_precoeff1(nullptr), sf_precoeff2(nullptr), sf_precoeff3(nullptr), sf_precoeff4(nullptr), sf_precoeff5(nullptr),
      sf_precoeff6(nullptr), acons(nullptr), density_A_brick(nullptr), density_B_brick(nullptr), density_A_fft(nullptr), density_B_fft(nullptr), fft1(nullptr),
      fft2(nullptr), remap(nullptr), gc(nullptr), gc_buf1(nullptr), gc_buf2(nullptr), part2grid(nullptr), boxlo(nullptr), GA2rho(nullptr), GA2grid(nullptr),
      SOL_density_brick(nullptr)
// GA_density_brick(nullptr)
{
  // Note that sv_GA_Flag is deprecated
  sv_GA_Flag = sv_SOL_Flag = sv_SOL_saved = false;
  sv_SOL_timestep                         = -1;
  fixed_flag                              = true;
  ago = -1, init_times = -1;
  ...
  sf_precoeff1 = sf_precoeff2 = sf_precoeff3 = sf_precoeff4 = sf_precoeff5 = sf_precoeff6 = nullptr;
...
void PPPM_conp_GA::settings(int narg, char** arg)
{
  me = comm->me;
  ...
  while (iarg < narg) {
    ...
    sv_GA option."); if (strcmp(arg[iarg+1],"on") == 0) { sv_GA_Flag = true; }
    ...
    available options are on or off"); iarg += 2;
      ...
      error->all(FLERR, "Illegal pppm_conp/GA command for symbol %.200s\n", arg[iarg]);
    }
  } // while iarg
  /*
  if (fixed_flag == true) {
...
PPPM_conp_GA::~PPPM_conp_GA()
{
  if (copymode) return;

  delete[] factors;
  PPPM_conp_GA::deallocate();
  if (peratom_allocate_flag) PPPM_conp_GA::deallocate_peratom();
  if (group_allocate_flag) PPPM_conp_GA::deallocate_groups();
  memory->destroy(part2grid);
  memory->destroy(acons);
  memory->destroy(GA2grid);
  memory->destroy2d_offset(GA2rho, nlower);
}
...
void PPPM_conp_GA::init()
{
  init_times++;
  if (init_times > 0 && fixed_flag) {
    gewaldflag = true;
    gridflag   = true;
  }
  if (me == 0) utils::logmesg(lmp, "PPPM initialization without OPT...\n");

  // error check

  triclinic_check();

  if (triclinic != domain->triclinic) error->all(FLERR, "Must redefine kspace_style after changing to triclinic box");
  ...
  if (slabflag) {
    if (domain->xperiodic != 1 || domain->yperiodic != 1 || domain->boundary[2][0] != 1 || domain->boundary[2][1] != 1)
      error->all(FLERR, "Incorrect boundaries with slab PPPM");
  }

  if (order < 2 || order > MAXORDER) error->all(FLERR, "PPPM order cannot be < 2 or > {}", MAXORDER);
  ...
  if (tip4pflag) {
    ...
    if (!p_qdist || !p_typeO || !p_typeH || !p_typeA || !p_typeB) error->all(FLERR, "Pair style is incompatible with TIP4P KSpace style");
    qdist     = *p_qdist;
    typeO     = *p_typeO;
    typeH     = *p_typeH;
    int typeA = *p_typeA;
    ...
    if (force->angle == nullptr || force->bond == nullptr || force->angle->setflag == nullptr || force->bond->setflag == nullptr)
      error->all(FLERR, "Bond and angle potentials must be defined for TIP4P");
    if (typeA < 1 || typeA > atom->nangletypes || force->angle->setflag[typeA] == 0) error->all(FLERR, "Bad TIP4P angle type for PPPM/TIP4P");
    if (typeB < 1 || typeB > atom->nbondtypes || force->bond->setflag[typeB] == 0) error->all(FLERR, "Bad TIP4P bond type for PPPM/TIP4P");
  ...
  while (order >= minorder) {
    ...
    gctmp = new GridComm(lmp, world, nx_pppm, ny_pppm, nz_pppm, nxlo_in, nxhi_in, nylo_in, nyhi_in, nzlo_in, nzhi_in, nxlo_out, nxhi_out, nylo_out, nyhi_out,
                         nzlo_out, nzhi_out);
...
double PPPM_conp_GA::map_make_rho_SOL(int groupbit)
{
  FUNC_MACRO(0)
  if (sv_SOL_Flag) {
    if (sv_SOL_saved == false || sv_SOL_timestep != update->ntimestep || update->ntimestep == update->firststep) {
      particle_map(groupbit, false);
      qSOL_local = make_rho(groupbit, false);
      copy_rho(SOL_density_brick, density_brick);
      sv_SOL_saved    = true;
      sv_SOL_timestep = update->ntimestep;
      ago             = neighbor->ago;
    } else {
      copy_rho(density_brick, SOL_density_brick);
    }
  } else {
    particle_map(groupbit, false);
    qSOL_local   = make_rho(groupbit, false);
...
void PPPM_conp_GA::setup_FIX(int groupbit)
{
  ago             = -1;
  sv_SOL_saved    = false;
  sv_SOL_timestep = true;
  FIX_groupbit0   = groupbit;
  setup_GA2rho();
}
/* ----------------------------------------------------------------------
   setup the invariant GA2rho: Map from GA particle to mesh
------------------------------------------------------------------------- */
void PPPM_conp_GA::setup_GA2rho()
{
  ...
  const int* const GA_seq_arr = &FIX->localGA_seq.front();
  // printf("localGA_num = %d\n", localGA_num);
  // memory->create3d_offset(GA2rho, 0, localGA_num-1, 0, 2, nlower, nupper,
  // "pppm:GA2rho");
  // TODO the order of PPPM must be invariant.
  if (GA2rho == nullptr) {
    memory->create2d_offset(GA2rho, 3 * localGA_num, nlower, nupper, "pppm:GA2rho");
  }
  if (GA2grid == nullptr) {
    memory->create(GA2grid, localGA_num * 3, "pppm:GA2grid");
  } else {
    // memory->grow(GA2grid, localGA_num * 3, "pppm:GA2grid");
  }

  for (iGA = 0; iGA < localGA_num; iGA++) {
    i  = GA_seq_arr[iGA];
    dx = (x[i][0] - boxlo[0]);
    dy = (x[i][1] - boxlo[1]);
    dz = (x[i][2] - boxlo[2]);
    ...
    dy *= delyinv;
    dz *= delzinv;
    nx              = static_cast<int>(dx + shift) - OFFSET;
    ny              = static_cast<int>(dy + shift) - OFFSET;
    nz              = static_cast<int>(dz + shift) - OFFSET;
    part2grid[i][0] = nx;
    part2grid[i][1] = ny;
    part2grid[i][2] = nz;
    dx              = nx + shiftone - dx;
    ...
    for (l = nlower; l <= nupper; ++l) {
      /*
      GA2rho[iGA][0][l] = rho1d[0][l];
      GA2rho[iGA][1][l] = rho1d[1][l];
      GA2rho[iGA][2][l] = rho1d[2][l];
      */
      GA2rho[iGA3][l]     = rho1d[0][l];
      GA2rho[iGA3 + 1][l] = rho1d[1][l];
      GA2rho[iGA3 + 2][l] = rho1d[2][l];
    }
  } // i [0, localGA_num);
}
...
void PPPM_conp_GA::setup_triclinic()
{
  int i, j, k, n;
  double* prd;
...
void PPPM_conp_GA::compute(int eflag, int vflag)
{
  FUNC_MACRO(0);
  int i, j;
  ...
  // include all particle
  /*
  particle_map(ALL_groupbit, true);
  clear_rho();
  make_rho(ALL_groupbit, true);
  */
  ...
  if (electro == nullptr) {
    particle_map(ALL_groupbit, true);
    make_rho(ALL_groupbit, true);
    // norm_mat_p(&density_brick[nzlo_out][nylo_out][nxlo_out], ngrid,
    // "after_SOL");
  } else {
    map_make_rho_SOL(FIX_groupbit0);
    // norm_mat_p(&density_brick[nzlo_out][nylo_out][nxlo_out], ngrid,
    // "before_SOL");
    make_rho_GA();
  }
  ...
  gc->reverse_comm(GridComm::KSPACE, this, 1, sizeof(FFT_SCALAR), REVERSE_RHO, gc_buf1, gc_buf2, MPI_FFT_SCALAR);
  brick2fft();
...
void PPPM_conp_GA::allocate()
{
  memory->create3d_offset(density_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:density_brick");
  if (sv_SOL_Flag) {
    memory->create3d_offset(SOL_density_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:SOL_density_brick");
  }

  // if(sv_GA_Flag) memory->create3d_offset( GA_density_brick,nzlo_out,
  //   nzhi_out,nylo_out,nyhi_out,nxlo_out,nxhi_out,"pppm:GA_density_brick");
  ...
  if (differentiation_flag == 1) {
    memory->create3d_offset(u_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:u_brick");
    ...
    memory->create3d_offset(vdx_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:vdx_brick");
    memory->create3d_offset(vdy_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:vdy_brick");
    memory->create3d_offset(vdz_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:vdz_brick");
  }
  ...
  fft1 = new FFT3d(lmp, world, nx_pppm, ny_pppm, nz_pppm, nxlo_fft, nxhi_fft, nylo_fft, nyhi_fft, nzlo_fft, nzhi_fft, nxlo_fft, nxhi_fft, nylo_fft, nyhi_fft,
                   nzlo_fft, nzhi_fft, 0, 0, &tmp, collective_flag);

  fft2 = new FFT3d(lmp, world, nx_pppm, ny_pppm, nz_pppm, nxlo_fft, nxhi_fft, nylo_fft, nyhi_fft, nzlo_fft, nzhi_fft, nxlo_in, nxhi_in, nylo_in, nyhi_in,
                   nzlo_in, nzhi_in, 0, 0, &tmp, collective_flag);

  remap = new Remap(lmp, world, nxlo_in, nxhi_in, nylo_in, nyhi_in, nzlo_in, nzhi_in, nxlo_fft, nxhi_fft, nylo_fft, nyhi_fft, nzlo_fft, nzhi_fft, 1, 0, 0,
                    FFT_PRECISION, collective_flag);

  // create ghost grid object for rho and electric field communication
  // also create 2 bufs for ghost grid cell comm, passed to GridComm methods

  gc = new GridComm(lmp, world, nx_pppm, ny_pppm, nz_pppm, nxlo_in, nxhi_in, nylo_in, nyhi_in, nzlo_in, nzhi_in, nxlo_out, nxhi_out, nylo_out, nyhi_out,
                    nzlo_out, nzhi_out);
...
void PPPM_conp_GA::deallocate()
{

  if (sv_SOL_Flag) {
    // printf("deallocate sv_SOL_Flag\n");
    memory->destroy3d_offset(SOL_density_brick, nzlo_out, nylo_out, nxlo_out);
  }
  // if (sv_GA_Flag)
  // memory->destroy3d_offset(GA_density_brick,nzlo_out,nylo_out,nxlo_out);
  // memory->destroy(GA2rho);
  // memory->destroy2d_offset(GA2rho, nlower);
...
void PPPM_conp_GA::allocate_peratom()
{
  peratom_allocate_flag = 1;

  if (differentiation_flag != 1) memory->create3d_offset(u_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:u_brick");

  memory->create3d_offset(v0_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v0_brick");

  memory->create3d_offset(v1_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v1_brick");
  memory->create3d_offset(v2_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v2_brick");
  memory->create3d_offset(v3_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v3_brick");
  memory->create3d_offset(v4_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v4_brick");
  memory->create3d_offset(v5_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:v5_brick");
...
void PPPM_conp_GA::deallocate_peratom()
{
  peratom_allocate_flag = 0;
...
void PPPM_conp_GA::set_grid_global()
{
  ...
  if (nx_pppm >= OFFSET || ny_pppm >= OFFSET || nz_pppm >= OFFSET) error->all(FLERR, "PPPM grid is too large");
}
...
int PPPM_conp_GA::factorable(int n)
{
  int i;
...
double PPPM_conp_GA::compute_df_kspace()
{
  double xprd      = domain->xprd;
  double yprd      = domain->yprd;
  double zprd      = domain->zprd;
...
double PPPM_conp_GA::compute_qopt()
{
  int k, l, m, nx, ny, nz;
...
double PPPM_conp_GA::estimate_ik_error(double h, double prd, bigint natoms)
{
  double sum = 0.0;
  if (natoms == 0) return 0.0;
  for (int m = 0; m < order; m++)
    sum += acons[order][m] * pow(h * g_ewald, 2.0 * m);
  double value = q2 * pow(h * g_ewald, (double)order) * sqrt(g_ewald * prd * sqrt(MY_2PI) * sum / natoms) / (prd * prd);
...
void PPPM_conp_GA::adjust_gewald()
{
  double dx;
...
double PPPM_conp_GA::newton_raphson_f()
{
  double xprd   = domain->xprd;
  double yprd   = domain->yprd;
  double zprd   = domain->zprd;
  ...
  double df_rspace = 2.0 * q2 * exp(-g_ewald * g_ewald * cutoff * cutoff) / sqrt(natoms * cutoff * xprd * yprd * zprd);
...
double PPPM_conp_GA::final_accuracy()
{
  double xprd   = domain->xprd;
  double yprd   = domain->yprd;
  double zprd   = domain->zprd;
...
void PPPM_conp_GA::set_grid_local()
{
  ...
  comm->partition_grid(nx_pppm, ny_pppm, nz_pppm, slab_volfactor, nxlo_in, nxhi_in, nylo_in, nyhi_in, nzlo_in, nzhi_in);
...
void PPPM_conp_GA::compute_gf_denom()
{
  int k, l, m;
...
void PPPM_conp_GA::compute_gf_ik()
{
  ...
  const int nbx      = static_cast<int>((g_ewald * xprd / (MY_PI * nx_pppm)) * pow(-log(EPS_HOC), 0.25));
  const int nby      = static_cast<int>((g_ewald * yprd / (MY_PI * ny_pppm)) * pow(-log(EPS_HOC), 0.25));
  const int nbz      = static_cast<int>((g_ewald * zprd_slab / (MY_PI * nz_pppm)) * pow(-log(EPS_HOC), 0.25));
  const int twoorder = 2 * order;
...
void PPPM_conp_GA::compute_gf_ik_triclinic()
{
  double snx, sny, snz;
  double argx, argy, argz, wx, wy, wz, sx, sy, sz, qx, qy, qz;
  double sum1, dot1, dot2;
...
void PPPM_conp_GA::compute_gf_ad()
{
  const double* const prd = domain->prd;
...
void PPPM_conp_GA::compute_sf_precoeff()
{
  int i, k, l, m, n;
...
void PPPM_conp_GA::particle_map(int groupbit, bool include_flag)
{
  if (groupbit == 0 and include_flag == true) {
    return;
  }
  int nx, ny, nz;
  ...
  int flag  = 0;
  int* mask = atom->mask;
  if (!std::isfinite(boxlo[0]) || !std::isfinite(boxlo[1]) || !std::isfinite(boxlo[2])) error->one(FLERR, "Non-numeric box dimensions - simulation unstable");

  for (int i = 0; i < nlocal; i++) {
    if (((mask[i] & groupbit) != 0) != include_flag) continue;
    // order = even:
    ...
    if (nx + nlower < nxlo_out || nx + nupper > nxhi_out || ny + nlower < nylo_out || ny + nupper > nyhi_out || nz + nlower < nzlo_out ||
        nz + nupper > nzhi_out)
      flag = 1;
  }
...
void PPPM_conp_GA::compute_dipole(int groupbit, bool Select_Flag)
{
  double dipole = 0.0, dipole_r2 = 0.0;
  double qtmp, z;
  ...
  MPI_Allreduce(&dipole, &dipole_group, 1, MPI_DOUBLE, MPI_SUM, world);
  MPI_Allreduce(&dipole_r2, &dipole_r2_group, 1, MPI_DOUBLE, MPI_SUM, world);
}

void PPPM_conp_GA::compute_SOLtoGA(bool GG_Flag)
{
  FUNC_MACRO(0);
  clear_rho();
  FixConpGA* FIX    = electro->FIX;
  double qsum_local = 0.0, qsum_group;
  if (GG_Flag == 1) {
    particle_map(ALL_groupbit, true);
    qsum_local = make_rho(ALL_groupbit, true);
    // qsum_local = map_make_rho_SOL(FIX_groupbit0);
    // qsum_local += make_rho(FIX_groupbit0, true);
    if (slabflag == 1) compute_dipole(ALL_groupbit, true);
  } else if (GG_Flag == 0) {
    // particle_map(FIX_groupbit, false);
    // qsum_local = make_rho(FIX_groupbit, false);
    qsum_local = map_make_rho_SOL(FIX_groupbit0);
    if (slabflag == 1) compute_dipole(FIX_groupbit0, false);
  }
  MPI_Allreduce(&qsum_local, &qsum_group, 1, MPI_DOUBLE, MPI_SUM, world);

  calc_GA_POT(qsum_group, GG_Flag);
}

void PPPM_conp_GA::compute_GAtoGA()
{
  FUNC_MACRO(0);
  clear_rho();
  FixConpGA* FIX = electro->FIX;
  double qsum_local, qsum_group;
  qsum_local = make_rho_GA();
  MPI_Allreduce(&qsum_local, &qsum_group, 1, MPI_DOUBLE, MPI_SUM, world);
  if (slabflag == 1) compute_dipole(FIX_groupbit0, true);
  calc_GA_POT(qsum_group, true);
}

void PPPM_conp_GA::calc_GA_POT(double qsum_group, bool GG_Flag)
{
  FUNC_MACRO(0);
  /*
  evflag = 0;
  ...
  */
  ...
  /* Poisson of potential from SOL particles */
  // particle_map(ALL_groupbit, true);
  ...
  for (iGA = 0; iGA < localGA_num; iGA++) {
    ...
    i    = GA_seq_arr[iGA];
    iGA3 = iGA * 3;
    nx   = GA2grid[iGA3];     // part2grid[i][0];
    ny   = GA2grid[iGA3 + 1]; // part2grid[i][1];
    nz   = GA2grid[iGA3 + 2]; // part2grid[i][2];

    u = ZEROF;
    for (n = nlower; n <= nupper; n++) {
      mz = n + nz;
      z0 = GA2rho[iGA3 + 2][n]; // rho1d[2][n];
      for (m = nlower; m <= nupper; m++) {
        my = m + ny;
        y0 = z0 * GA2rho[iGA3 + 1][m]; // rho1d[1][m];
        for (l = nlower; l <= nupper; l++) {
          mx = l + nx;
          x0 = y0 * GA2rho[iGA3][l]; //;
    ...
    // slab correlation
    if (slabflag == 1) eatom[i] += efact * (x[i][2] * dipole_group - 0.5 * (dipole_r2_group + qsum_group * x[i][2] * x[i][2]) - zprd_const);
    /*
    if (i < 10) {
      printf("i = %d. eatom[i] = %.12g\n", i, eatom[i]);
    }
    */
  }
  // print_hash(eatom, localGA_num, "eatom[i]");
  ...
  if (slabflag == 1) {
    ...
    double dipole = 0.0, dipole_r2 = 0.0;
    double dipole_all, dipole_r2_all;
    ...
    MPI_Allreduce(&dipole_r2,&dipole_r2_all,1,MPI_DOUBLE,MPI_SUM,world);


    for (iGA = 0; iGA < localGA_num; iGA++) {
      i = GA_seq_arr[iGA];
      eatom[i] += efact *(x[i][2]*dipole_group - 0.5*(dipole_r2_group +
            qsum_group*x[i][2]*x[i][2]) - zprd_const);
    }
  }
  */
  ...
  // printf("GA2grid = %x, GA2rho = %x, eatom = %x\n", GA2grid, GA2rho, eatom);
  int l,m,n,nx,ny,nz,mx,my,mz;
  FFT_SCALAR dx,dy,dz,x0,y0,z0;
  double etmp, *rho1d_ptr;
  int iGA, i, iGA3;
  ...
  for (iGA = 0; iGA < localGA_num; iGA++) {
    ...
    etmp = 0;
    iGA3 = iGA * 3;
    nx = part2grid[i][0]; //GA2grid[iGA3]  ;
    ny = part2grid[i][1]; //GA2grid[iGA3+1];
    nz = part2grid[i][2]; //GA2grid[iGA3+2];
    rho1d_ptr = GA2rho[iGA3];
    for (n = nlower; n <= nupper; n++) {
      mz = n;
      z0 = GA2rho[iGA3+2][n];
      for (m = nlower; m <= nupper; m++) {
        my = m+ny;
        y0 = z0*GA2rho[iGA3+1][m];;
        for (l = nlower; l <= nupper; l++) {
          mx = l+nx;
          x0 = y0*GA2rho[iGA3][l];
          etmp = x0*u_brick[mz][my][mx];
        }
      }
    } // n
    // eatom[0] = etmp;
...
/* ----------------------------------------------------------------------
   ...
   in global grid
------------------------------------------------------------------------- */
void PPPM_conp_GA::clear_rho()
{
  // clear 3d density array
  memset(&(density_brick[nzlo_out][nylo_out][nxlo_out]), 0, ngrid * sizeof(FFT_SCALAR));
}

inline void PPPM_conp_GA::copy_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src)
{
  memcpy(&(dst[nzlo_out][nylo_out][nxlo_out]), &(src[nzlo_out][nylo_out][nxlo_out]), ngrid * sizeof(FFT_SCALAR));
}

void PPPM_conp_GA::add_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src)
{
  for (int nz = nzlo_out; nz <= nzhi_out; ++nz)
    for (int ny = nylo_out; ny <= nyhi_out; ++ny)
      for (int nx = nxlo_out; nx <= nxhi_out; ++nx) {
        dst[nz][ny][nx] += src[nz][ny][nx];
      }
}

void PPPM_conp_GA::sub_rho(FFT_SCALAR*** dst, FFT_SCALAR*** src)
{
  for (int nz = nzlo_out; nz <= nzhi_out; ++nz)
    for (int ny = nylo_out; ny <= nyhi_out; ++ny)
      for (int nx = nxlo_out; nx <= nxhi_out; ++nx) {
...
/* ------------------------------------------------------------------
     make rho from the fixed GA particles
------------------------------------------------------------------  */
double PPPM_conp_GA::make_rho_GA()
{
  FUNC_MACRO(0);
  int l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz, x0, y0, z0;
  ...
  int iGA, iGA3;
  int* GA_seq_arr = &FIX->localGA_seq.front();
  double *q = atom->q, qsum = 0.0;

  FFT_SCALAR* rho1d_ptr;
  if (FIX->firstgroup_flag) {
    for (iGA = 0; iGA < localGA_num; iGA++) {
      ...
      for (n = nlower; n <= nupper; n++) {
        mz = n + nz;
        y0 = z0 * GA2rho[iGA3 + 2][n];
        for (m = nlower; m <= nupper; m++) {
          my = m + ny;
          x0 = y0 * GA2rho[iGA3 + 1][m];
          for (l = nlower; l <= nupper; l++) {
            mx = l + nx;
            density_brick[mz][my][mx] += x0 * rho1d_ptr[l];
          }
        }
      }
    } // iGA in [0, localGA_num)
  } else {
    for (iGA = 0; iGA < localGA_num; iGA++) {
      // printf("iGA = %d - %d\n", iGA, i);
      int i = GA_seq_arr[iGA];
      qsum += q[i];
...
/* ------------------------------------------------------------------
     make rho from the particle w/wo the groupbit by include_flag
------------------------------------------------------------------  */
double PPPM_conp_GA::make_rho(int groupbit, bool include_flag)
{
  FUNC_MACRO(0);
  if (groupbit == 0 && include_flag == true) return 0;
  int l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz, x0, y0, z0;
  ...
  // (dx,dy,dz) = distance to "lower left" grid pt
  // (mx,my,mz) = global coords of moving stencil pt
  double qsum_group = 0.0;
  double* q         = atom->q;
  double** x        = atom->x;
  int nlocal = atom->nlocal, i, icount = 0;
  int* mask = atom->mask;
...
void PPPM_conp_GA::poisson_ik_triclinic()
{
  int i, j, k, n;
...
void PPPM_conp_GA::poisson_peratom()
{
  int i, j, k, n;
...
void PPPM_conp_GA::fieldforce_ik()
{
  int i, l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz, x0, y0, z0;
...
void PPPM_conp_GA::fieldforce_ad()
{
  int i, l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz;
  FFT_SCALAR ekx, eky, ekz;
...
void PPPM_conp_GA::fieldforce_peratom()
{
  int i, l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz, x0, y0, z0;
  FFT_SCALAR u, v0, v1, v2, v3, v4, v5;
...
void PPPM_conp_GA::pack_forward_grid(int flag, void* vbuf, int nlist, int* list)
{
...
void PPPM_conp_GA::unpack_forward_grid(int flag, void* vbuf, int nlist, int* list)
{
  FFT_SCALAR* buf = (FFT_SCALAR*)vbuf;
...
void PPPM_conp_GA::pack_reverse_grid(int flag, void* vbuf, int nlist, int* list)
{
  FFT_SCALAR* buf = (FFT_SCALAR*)vbuf;
...
void PPPM_conp_GA::unpack_reverse_grid(int flag, void* vbuf, int nlist, int* list)
{
  FFT_SCALAR* buf = (FFT_SCALAR*)vbuf;
...
void PPPM_conp_GA::procs2grid2d(int nprocs, int nx, int ny, int* px, int* py)
{
...
void PPPM_conp_GA::compute_rho1d(const FFT_SCALAR& dx, const FFT_SCALAR& dy, const FFT_SCALAR& dz)
{
  int k, l;
  FFT_SCALAR r1, r2, r3;
...
void PPPM_conp_GA::compute_drho1d(const FFT_SCALAR& dx, const FFT_SCALAR& dy, const FFT_SCALAR& dz)
{
  int k, l;
  FFT_SCALAR r1, r2, r3;
...
void PPPM_conp_GA::compute_rho_coeff()
{
  int j, k, l, m;
  FFT_SCALAR s;
...
void PPPM_conp_GA::slabcorr()
{
  ...
  const double e_slabcorr = MY_2PI * (dipole_all * dipole_all - qsum * dipole_r2 - qsum * qsum * zprd * zprd / 12.0) / volume;
  const double qscale     = qqrd2e * scale;
  ...
  if (eflag_atom) {
    ...
    for (int i = 0; i < nlocal; i++)
      eatom[i] += efact * q[i] * (x[i][2] * dipole_all - 0.5 * (dipole_r2 + qsum * x[i][2] * x[i][2]) - qsum * zprd * zprd / 12.0);
  }
...
int PPPM_conp_GA::timing_1d(int n, double& time1d)
{
  double time1, time2;
...
int PPPM_conp_GA::timing_3d(int n, double& time3d)
{
  double time1, time2;
...
double PPPM_conp_GA::memory_usage()
{
  double bytes = (double)nmax * 3 * sizeof(double);
  int nbrick   = (nxhi_out - nxlo_out + 1) * (nyhi_out - nylo_out + 1) * (nzhi_out - nzlo_out + 1);
  if (differentiation_flag == 1) {
  ...
  if (peratom_allocate_flag) bytes += (double)6 * nbrick * sizeof(FFT_SCALAR);
...
void PPPM_conp_GA::compute_group_group(int groupbit_A, int groupbit_B, int AA_flag)
{
...
void PPPM_conp_GA::allocate_groups()
{
  group_allocate_flag = 1;

  memory->create3d_offset(density_A_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:density_A_brick");
  memory->create3d_offset(density_B_brick, nzlo_out, nzhi_out, nylo_out, nyhi_out, nxlo_out, nxhi_out, "pppm:density_B_brick");
...
void PPPM_conp_GA::deallocate_groups()
{
  group_allocate_flag = 0;
...
void PPPM_conp_GA::make_rho_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  int l, m, n, nx, ny, nz, mx, my, mz;
  FFT_SCALAR dx, dy, dz, x0, y0, z0;
  ...
  memset(&(density_A_brick[nzlo_out][nylo_out][nxlo_out]), 0, ngrid * sizeof(FFT_SCALAR));

  memset(&(density_B_brick[nzlo_out][nylo_out][nxlo_out]), 0, ngrid * sizeof(FFT_SCALAR));
      ...
      for (n = nlower; n <= nupper; n++) {
        ...
        for (m = nlower; m <= nupper; m++) {
          ...
          for (l = nlower; l <= nupper; l++) {
            ...
            if (mask[i] & groupbit_A) density_A_brick[mz][my][mx] += x0 * rho1d[0][l];

            // group B

            if (mask[i] & groupbit_B) density_B_brick[mz][my][mx] += x0 * rho1d[0][l];
          }
        }
      }
    }
...
void PPPM_conp_GA::poisson_groups(int AA_flag)
{
  int i, j, k, n;
  ...
  n = 0;
  for (i = 0; i < nfft; i++) {
    e2group += s2 * greensfn[i] * (work_A[n] * work_B[n] + work_A[n + 1] * work_B[n + 1]);
    n += 2;
  }
...
void PPPM_conp_GA::poisson_groups_triclinic()
{
  int i, n;
...
void PPPM_conp_GA::slabcorr_groups(int groupbit_A, int groupbit_B, int AA_flag)
{
  // compute local contribution to global dipole moment
  ...
  e2group += efact * (dipole_A * dipole_B - 0.5 * (qsum_A * dipole_r2_B + qsum_B * dipole_r2_A) - qsum_A * qsum_B * zprd * zprd / 12.0);
